...
cp .env.example .env               # fill in API keys and contact/recipient emails
cp profile.example.py profile.py   # then edit profile.py with your own reader profile
```

`profile.py` is gitignored -- it holds who you are and what you care about,
so it never ends up committed. `config.py` holds everything not sensitive.
...
Every run is logged to `run.log`. Each stage also runs standalone:
...
| `config.py` | Feed list + global settings |
